...
#'   manipulation.
#'
#' @examples plotAlleleFreq(genotypes)
#' p <- plotAlleleFreq(genotypes)
...
plotAlleleFreq <- function(genos = "genotypes"){
  ...
  ####get the number of A,B,H for each marker, apply doesnt work for some dont have N####
  ...
  genotypeAbs <- melt(as.data.frame(genotypeAbs),id.vars = c("chrom","pos"))
  ...
  genotypeRatio <- melt(as.data.frame(genotypeRatio),id.vars = c("chrom","pos"),
  ...
  ggplot(data = genotypeRatio, aes(x = pos/1000000, y = value, color = allele_state))+
    geom_line(size = 0.75)+
    #geom_hline(y = 50, alpha = 0.5, color = "black", linetype = "dashed")+
    facet_grid(chrom~.)+
